...
class Net(nn.Module):
    def __init__(self):
        super(Net, self).__init__()
        self.regressor = nn.Sequential(nn.Linear(2, NN),
                                       nn.Tanh(),
                                       nn.Linear(NN, NN),
                                       nn.Tanh(),
                                       nn.Linear(NN, NN),
...
#u(t,1) = u(t,0)
u_t_bc = np.zeros((1,100))
t_t_bc = np.linspace(0,2*np.pi,100)
#u(0,x)
u_t0_x = np.linspace(0,2*np.pi,num = 100)
...
for epoch in range(EPOCHS):
    ...
    net_bc_x = torch.autograd.grad(net_bc_out.sum(), pt_t_bc, create_graph=True)[0]
    mse_u = criterion(input = net_bc_out, target = pt_x_bc)+ criterion(input = net_bc_x, target = pt_x_bc_t)# Boundary loss
